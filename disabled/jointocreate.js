...
module.exports = function (client) {
  ...
  console.log('jointocreate support WIP')
  console.log(`--------------------------`);
  ...
  client.on("voiceStateUpdate", (oldState, newState) => {
    ...
    if (!oldState.channelID && newState.channelID) {
      if (newState.channelID !== config.JOINTOCREATECHANNEL1) return;  //if its not the jointocreatechannel skip
    ...
    if (oldState.channelID && newState.channelID) {
      ...
      if (oldState.channelID !== newState.channelID) {
        ...
        if (newState.channelID === config.JOINTOCREATECHANNEL1)
          //make a new channel
  ...
  async function jointocreatechannel(user) {
    ...
    //user.member.user.send("This can be used to message the member that a new room was create

    await user.guild.channels.create(`Support Rum`, {//${user.member.user.username}'s Room
      type: 'voice',
  ...
  async function moveChannel(vc) {
    await vc.setParent("906219205685301249");
  };
}